...
3. **Moss Semantic Intent Classifier**: Routes user symptoms (e.g., *"washing machine vibrating violently"*) to custom troubleshooting checklists in under 10ms.
...
When a user types a symptom (e.g., *"air conditioner blowing hot air"*), Hamic queries the `hamic-support-docs` index. Moss classifies the user's intent and maps it to a specific structured diagnostic checklist (e.g. `cooling` or `spinning`) in under 10ms. If Moss is unavailable or limits are reached, the system falls back to category-based matching.

### 2. Spare Parts Retrieval RAG (`lib/mossParts.ts`)
Once the troubleshooting assistant isolates the root cause (e.g., *"unbalanced load"* or *"refrigerant leak"*), Moss semantic query searches the parts catalog. It retrieves highly relevant tools (like a multimeter or socket wrench) and spare parts (like a drive belt or HEPA filter) matching the isolated cause and links them directly to the storefront.
